...
class _uploader
{
				...
				if(is_array($v['error'])) {
					foreach($v['error'] as $k2 => $err) {
						...
						if($err) {
							...
							if($f = $this->uploadFile($v['tmp_name'][$k2],$filename, $name)) {
								$files[] = $f;
							}
						}
					}
					...
					if($v['error']) {
						...
						if($f = $this->uploadFile($v['tmp_name'], $filename, $name)) {
							$files[] = $f;
						}
					}
				}
	...
	function chkExt($filename)
	{
		...
		if($this->options->ext && is_array($this->options->ext)) {
			...
			if(!in_array(strtolower($info['extension']), $exts)) {
				$this->error = "File Extension: .{$info['extension']}  is not supported. Please choose a file with valid('.pdf', '.ppdf', '.pptx', '.docx', '.doc') extension";
				return false;
			}
		}
		return true;
	 ...
	 * @param unknown_type $filename
	 */
	function uploadFile($tmp_name, $filename, $name)
	{
		$info = pathinfo($filename);
		...
		if(@move_uploaded_file($tmp_name,$this->options->target.$filename)) {
                        ...
                        //$ajax->location('http://www.google.com');
			//$this->upload_count++;
			//return $filename;
		} else {
			sleep(2);
