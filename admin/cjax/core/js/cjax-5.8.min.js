...
{if(typeof buffer=='object'){buffer=buffer.buffer;}
...
CJAX.lib.fnCallback(my_fn,data.a,data.b,data.c,data);}else{if(!data.c){if(data.b){if(CJAX.lib.isFN(my_fn)){if(type=='window'){window[_raw_fn][data.a](data.b);}}}}else{arr_data=[];for(x in data){arr_data.push(data[x]);}
my_fn.apply(this,arr_data);}}}
...
break;case'LABEL':case'BUTTON':case'DIV':case'SPAN':case'THEAD':case'TH':case'TR':case'TD':case'TBODY':case'TFOOT':element.innerHTML=value;break;case'INPUT':{switch(element.type){case'button':case'radio':case'text':case'select-one':case'textarea':case'hidden':case'password':element.value=value;break;case'checkbox':element.checked=parseInt(value)?true:false;}}
...
var script;var name='cjax-5.8.js';var src;script=CJAX.$('cjax_lib');if(script){src=script.src;var f=src.replace(/cjax-.+$/,'');CJAX.uri=script.src;}else{var scripts=document.getElementsByTagName('script');for(var i=0;i<scripts.length;i++){script=scripts[i];src=script.src;if(CJAX.util.get.basename(src).indexOf('cjax-')!=-1){var f=src.replace(/cjax-.+$/,'');CJAX.uri=src;return f;}}}
...
return;}
if(typeof Sizzle!='undefined'||typeof jQuery!='undefined'){if(!/[^a-zA-Z0-9_\-]/.test(element)&&element.indexOf('#')==-1){element='#'+element;}
if(typeof jQuery!='undefined'){elements=jQuery(element);elements.each(function(index,element){CJAX.__AddEventTo(element,buffer);});}else{elements=Sizzle(element);for(x in elements){CJAX.__AddEventTo(elements[x],buffer);}}}else{CJAX.__AddEventTo(element,buffer);}};this.__AddEventTo=function(element,buffer)
{var events=CJAX.util.json(CJAX.xml('events',buffer));var event=CJAX.xml('event',buffer);for(x in events){buffer=events[x];CJAX.set.event(element,buffer.event?buffer.event:event,buffer.xml,x);}}
this.is_cjax=function(buffer){if(typeof buffer!='string'){return;}
...
_fn=function(){if(!_stop){CJAX._extendPlugin(plugin_name,method,{element:element,event:_event,element_id:element.id,clear:function(){CJAX._EventCache.flushElement(element);}});}else{_stop=false;}};}else{_fn=function(data,selector){_method=data.buffer.replace(/\n/g,"");if(!_stop){if(CJAX.util.isXML(_method)){if(!CJAX.is_cjax(_method)){_method="<cjax>"+_method+"</cjax>";}
CJAX._process({buffer:_method,selector:selector},'set.event',element);}else{eval(_method);}}else{_stop=false;}};}
...
_x[cache_id]={selector:element,buffer:method};return CJAX._addEvent(element,_event,function(){new_fn(_x[cache_id],element);},cache_id);},type:function(elem,new_type,verbose){if(!elem)return false;var elem=CJAX.is_element(elem,verbose);if(elem){elem.type=new_type;return true;}
...
{buffer1=CJAX.xml(tag,buffer);buffer1='<'+tag+'>'+buffer1+'</'+tag+'>';new_buffer=buffer.replace(buffer1,'');return new_buffer;};this.process=function(buffer,obj_buffer,caller,uniqid)
{if(!CJAX.defined(caller)){var caller='unknown';}
if(!CJAX.is_cjax(buffer)){alert('no cjax - caller: '+caller+'\n'+buffer);return false;};CJAX._process(buffer,obj_buffer,caller,uniqid);};this._process=function(buffer,obj_buffer,caller,uniqid)
{var obj_buffer=buffer;if(typeof buffer=='object'){buffer=buffer.buffer;}
...
if(CJAX[SUBFIX]){try{stack=CJAX.xml('stack',buffer);if(seconds){buffer=buffer.replace(/[\n\r]/gm,'\\n');}
...
if(seconds){setTimeout(PREFIX+CJAX.method+'("'+buffer+','+obj_buffer+'")',seconds);}else{fn=CJAX[SUBFIX];fn(buffer,obj_buffer);if(stack){CJAX.process_all(stack);}}}catch(_e){alert('#process unabled to load function#1: '+CJAX.method+'();  '+_e.message);}
...
return url.replace(/^&/,'');};this._form=function(buffer,obj_buffer){var selector;if(obj_buffer){selector=obj_buffer.selector;}
buffer=CJAX.decode(buffer);var url=CJAX.xml('url',buffer);var form_id=CJAX.xml('form_id',buffer);var args=CJAX.xml('args',buffer);var container=CJAX.xml('container',buffer);var text=CJAX._text=CJAX.xml('text',buffer);CJAX.IS_POST=is_post=CJAX.xml('post',buffer);if(text){CJAX.loading(text,true);}
...
serial=CJAX._serialize(form);url=CJAX._pharseValues(url,selector);if(container){container=CJAX.$(container);if(!container)return false;}
...
if(url){handlerFormRequest();CJAX.loading();}}else{if(url){handlerFormRequest();}}};this.refresh=function()
...
CJAX.IS_POST=false;return CJAX.call(options,options.success);};this._call=function(buffer,obj_buffer,callback){if(CJAX.debug){console.log('Call executed.');}
var selector;if(typeof obj_buffer=='object'){selector=obj_buffer.selector;}
...
CJAX.$('cjax_overlay').style.display='none';}
url=CJAX._pharseValues(url,selector);if(CJAX.ajaxSettings.stop){CJAX.ajaxSettings.stop=false;return true;}
...
CJAX.HTTP_REQUEST_INSTANCE.setRequestHeader('X-Requested-With','CJAX FRAMEW0RK 5.8');return full_url;};this._handlerRequestStatus=function(url,status,container)
...
if(typeof seconds=='undefined'){var seconds=CJAX.default_timeout;}
options={message:"<data><div class='cjax_message cjax_message_type cjax_error'>"+message+"</div></data>",time:seconds};CJAX._message(options);};this._pharseValues=function(url,selector)
{var v=url.match(/\|[^\|\|]*\|/g);try{for(x in v){id=v[x].replace(/\|/g,'');if(id=='rand'){element=CJAX.$(id);if(!element){_value=new Date().getTime();}else{_value=element.value;}}else{var use_fns=false;if(selector&&(id.indexOf('.')!=-1||id.indexOf('function')!=-1)){var data=id.split('.');switch(data[0]){case'this':_value=selector[data[1]];break;case'data':_value=CJAX.decode(selector.getAttribute('data-'+data[1]));break;default:var fn=CJAX.lib.pharseFunction(data[0]);if(CJAX.lib.isFn(fn)){_value=fn();}}}else{if(id.indexOf(':')!=-1){use_fns=true;fns=id.split(':');id=fns[0];element=CJAX.$(id);}else{element=CJAX.$(id);}
if(!element){continue;}
